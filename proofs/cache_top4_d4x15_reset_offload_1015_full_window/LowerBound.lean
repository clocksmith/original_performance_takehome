...
namespace ProofCacheTop4D4x15ResetOffload1015FullWindow
open ProofCommon

def GUARD : Nat := 0

def totalCycles : Nat := 1174
...
-- Partial caching parameters for 1015 witness.
...
lemma flow_capacity_1015 : flowOps ≤ FLOW_CAP * totalCycles := by
...
lemma load_capacity_1015 : totalLoadOps ≤ LOAD_CAP * totalCycles := by
  simp [totalLoadOps_eq, LOAD_CAP, totalCycles]

def storeOps : Nat := VECTORS -- vstore values once

lemma store_capacity_1015 : storeOps ≤ STORE_CAP * totalCycles := by
  simp [storeOps, STORE_CAP, VECTORS, totalCycles]

/-! ## Reset + setup accounting (1015 tuple) -/

def RESET_VALU_OPS_PER_VEC : Nat := 1
def EXTRA_ADDR_VALU_OPS : Nat := 241
def SETUP_VALU_OPS : Nat := 45

def totalVALU1015 : Nat :=
  totalVALU + VECTORS * RESET_VALU_OPS_PER_VEC + EXTRA_ADDR_VALU_OPS + SETUP_VALU_OPS

lemma totalVALU1015_eq : totalVALU1015 = 7870 := by
  simp [totalVALU1015, totalVALU_eq, VECTORS, RESET_VALU_OPS_PER_VEC, EXTRA_ADDR_VALU_OPS, SETUP_VALU_OPS]

/-- Offload needed at totalCycles and its ALU feasibility. -/
def offloadNeeded (T : Nat) : Nat := totalVALU1015 - VALU_CAP * T

lemma offload_needed_1015 : offloadNeeded totalCycles = 826 := by
  simp [offloadNeeded, totalCycles, totalVALU1015_eq, VALU_CAP]

lemma offloadCap_1015 : offloadCap totalCycles = 1761 := by
  native_decide

lemma offload_feasible_1015 :
    offloadNeeded totalCycles ≤ offloadCap totalCycles := by
  have hcap : offloadCap totalCycles = 1761 := offloadCap_1015
  have hneeded : offloadNeeded totalCycles = 826 := offload_needed_1015
  have hle : 826 ≤ 1761 := by decide
  simpa [hcap, hneeded]

def BASE_ALU_OPS : Nat := 7463
def totalALUOps (T : Nat) : Nat := BASE_ALU_OPS + offloadNeeded T * VLEN

lemma alu_capacity_1015 : totalALUOps totalCycles ≤ ALU_CAP * totalCycles := by
  native_decide

/-- Combined schedule skeleton: capacity + flow + load + stagger. -/
theorem schedule_skeleton :
  offloadNeeded totalCycles = 826 ∧
  ...
  totalCycles = 1174 := by
  refine ⟨offload_needed_1015, offload_feasible_1015, alu_capacity_1015, flow_capacity_1015, load_capacity_1015, store_capacity_1015, rfl⟩
...
def allocALU (t : Nat) : Nat := if t < 1172 then 12 else if t = 1172 then 7 else 0
...
lemma sumAlloc_valu : sumAlloc allocVALU = 7044 := by native_decide
lemma sumAlloc_alu : sumAlloc allocALU = 14071 := by native_decide
...
theorem constructive_schedule_counts :
  sumAlloc allocVALU = 7044 ∧
  sumAlloc allocALU = 14071 ∧
...
lemma valu_exec_ops_eq : VALU_EXEC_OPS = 7044 := by
...
/-! ### Setup assumptions for 1015 (counted) -/
...
def ALU_OFFLOAD_OPS : Nat := 826

-- Offload predicate: first 826 VALU ops in opNumFull order.
...
lemma offload_lt_total {v r s : Nat} (h : Offload v r s) : opNumFull v r s < totalVALU := by
  have h' : ALU_OFFLOAD_OPS < totalVALU := by
    -- 826 < 7552
    simpa [ALU_OFFLOAD_OPS, totalVALU_eq] using (by decide : 826 < 7552)
...
lemma alu_cycle_lt {k : Nat} (hk : k < ALU_OFFLOAD_OPS) : aluCycle k < VALU_CYCLES := by
  -- k < 826 ⇒ k/6 ≤ 825/6 = 137 < 1174
  have hle : k ≤ 825 := Nat.lt_succ_iff.mp hk
  have hdiv : k / VALU_CAP ≤ 825 / VALU_CAP := Nat.div_le_div_right hle
  have hmax : 825 / VALU_CAP = 137 := by native_decide
  have hbound : k / VALU_CAP ≤ 137 := by simpa [hmax] using hdiv
  exact lt_of_le_of_lt hbound (by decide : 137 < VALU_CYCLES)
...
end ProofCacheTop4D4x15ResetOffload1015FullWindow
