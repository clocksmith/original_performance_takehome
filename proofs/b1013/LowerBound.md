Feasible 1013 upper‑bound witness (full‑window model)
...
- Setup VALU ops are fixed at SETUP_VALU_OPS = 0 (pre‑initialized).
...
- Selection is ISA‑legal via flow vselect trees.
...
At T = 1013 cycles:
- VALU capacity = 6T = 6078 ops
- ALU offload capacity = floor(12T/8) = 1519 VALU‑equivalent ops
- Needed offload = 7584 − 6078 = 1506 (bitwise op1 stages)
...
- Flow ops: baseline top‑4 cached selection costs 22 vselects/vector ⇒ 704.
  ...
  With X4=15:
  flowOps = 704 + 64 + 15*15 = 993 ≤ 1013.
- Load ops: formula is
  totalLoadOps = 64 + PRELOAD_NODES + (16−8−DEPTH4_ROUNDS)*256 + DEPTH4_ROUNDS*(32−X4)*8.
  For X4=15, DEPTH4_ROUNDS=1 (round 4 cached only):
  totalLoadOps = 64 + 31 + 7*256 + (32−15)*8 = 2023 ≤ 2*1013.

Engine budgets at T=1013:
Engine | Required | Capacity | Notes
VALU   | 6078 ops | 6078 ops | 7584 total minus 1506 offloaded
ALU    | 12048 ops | 12156 ops | 1506 bitwise op1s × 8 lanes
Flow   | 993 ops | 1013 ops | vselect tree selection + setup
Load   | 2023 ops | 2026 ops | cached top‑4 + partial depth‑4 (X4=15)
Store  | 32 ops   | 2026 ops | vstore values once

Constructive count‑schedule (formalized in Lean):
We give explicit per‑cycle allocations over T=1013 cycles:
- VALU: 6 ops for cycles 0–1012 (total 6078).
- ALU: 12 ops for cycles 0–1003 (total 12048).
- Flow: 1 op for cycles 0–992 (total 993).
- Load: 2 ops for cycles 0–1010, 1 op on cycle 1011 (total 2023).
...
This yields a tight 1013 bound under the stated assumptions, with an explicit ISA‑legal
selection cost and load budget.
