...
namespace ProofCacheTop4D4x15ResetOffload1016ParityOffIdxshiftPtralu
open ProofCommon
...
def totalVALU : Nat := 7486

lemma valu_capacity : totalVALU ≤ VALU_CAP * totalCycles := by
  native_decide

/-- Base ALU ops (selection + address compute + parity offload + pointer setup). -/
def totalALUOps : Nat := 11112
...
end ProofCacheTop4D4x15ResetOffload1016ParityOffIdxshiftPtralu
