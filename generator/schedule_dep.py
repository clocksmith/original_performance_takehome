...
def schedule_ops_dep(ops: list[Op]) -> list[dict[str, list[tuple[Any, ...]]]]:
    n_ops = len(ops)
    ...
    deps: list[list[int]] = [list() for _ in range(n_ops)]
    ...
    for i in range(n_ops):
        ...
        for addr in reads:
            if addr in last_write:
                deps[i].append(last_write[addr])
        for addr in writes:
            if addr in last_write:
                deps[i].append(last_write[addr])
            if addr in last_read:
                deps[i].append(last_read[addr])
        for addr in reads:
            last_read[addr] = i
        for addr in writes:
            last_write[addr] = i
    ...
    children: list[list[int]] = [[] for _ in range(n_ops)]
    indegree = [0] * n_ops
    for i in range(n_ops):
        for d in deps[i]:
            children[d].append(i)
    ...
    for i in range(n_ops - 1, -1, -1):
        if children[i]:
            priority[i] = 1 + max(priority[c] for c in children[i])
    ...
    while remaining > 0:
        ...
        for engine in engine_order:
            ...
            while heap and count < cap:
                ...
                scheduled[idx] = cycle
                scheduled_this_cycle.append(idx)
        ...
        for idx in scheduled_this_cycle:
            for child in children[idx]:
                indegree[child] -= 1
                earliest[child] = max(earliest[child], scheduled[idx] + 1)
                if indegree[child] == 0:
